...
test.describe('Продвинутые CSS-селекторы', () => {
  ...
  test('Комбинированные условия поиска', async ({ page }) => {
    ...
    //    - Не disabled
    const submitButton = page.getByRole('button', { name: 'В корзину' }).nth(1); // TODO(student): замените на корректный локатор
...
test.describe('Динамический контент с условиями', () => {
  ...
  test('Фильтрация динамических элементов', async ({ page }) => {
    ...
    const dynamicButton = page.getByRole('button', { name: 'Недоступно' }); // TODO(student): замените на корректный локатор
  ...
  test('Комбинации с :has и :not', async ({ page }) => {
    ...
    //const availableProducts = page.getByRole('button', { name: 'В корзину' }); // TODO(student): замените на корректный локатор
    const availableProducts = page.locator(
      '.product-card:not(.sold-out):has(button:has-text("В корзину"))',
    );

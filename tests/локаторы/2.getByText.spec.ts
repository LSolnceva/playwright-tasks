...
test.describe('Сложные случаи поиска по тексту', () => {
  ...
  test('Найти вложенный текст', async ({ page }) => {
    const nestedSpan = page.getByText('Параграф с вложенным текстом внутри'); // TODO(student): замените на корректный локатор
  ...
  test('Найти текст с пробелами', async ({ page }) => {
    const spacedText = page.getByText(/с множественными пробелами/); // TODO(student): замените на корректный локатор

...
test.describe('Базовые тесты для getByAltText()', () => {
  ...
  test('Найти изображение по точному alt-тексту', async ({ page }) => {
    const landscapeImage = page.getByRole('img', { name: 'Красивый пейзаж с горами и озером' }); // TODO(student): замените на корректный локатор
    await expect(landscapeImage).toBeVisible();
    await expect(landscapeImage).toHaveJSProperty('width', 182);
  });
  ...
  test('Найти логотип компании', async ({ page }) => {
    const logo = page.getByAltText('Логотип компании ТехноКорп'); // TODO(student): замените на корректный локатор
...
test.describe('Тесты для динамических изображений', () => {
  ...
  test('Найти динамически загруженное изображение', async ({ page }) => {
    const dynamicImage = page.getByRole('heading', { name: 'Динамически загружаемые изображения' }); // TODO(student): замените на корректный локатор
    await expect(dynamicImage).toBeVisible({ timeout: 2000 });
    //await expect(dynamicImage).toHaveJSProperty('width', 400);
  });
  ...
  test('Найти все иконки по частичному alt-тексту', async ({ page }) => {
    const icons = page.getByRole('img', { name: /иконк/i }); // TODO(student): замените на корректный локатор

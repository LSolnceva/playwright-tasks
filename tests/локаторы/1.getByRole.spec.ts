...
test.describe('Поиск элементов форм по ролям', () => {
  ...
  test('Заполнить и отправить форму', async ({ page }) => {
    ...
    await username.fill('тестовый_пользователь');
    await password.fill('тестовый_пароль');
    await country.selectOption('Россия');
...
test.describe('Поиск вкладок и уведомлений по ролям', () => {
  ...
  test('Переключение между вкладками', async ({ page }) => {
    ...
    await page.getByRole('heading', { name: 'Настройки аккаунта' }).click();
    await settingsTab.click();
    await expect(settingsTab).toHaveAttribute('aria-selected', 'true');
    const settingsPanel = page.getByRole('tab', { name: 'Настройки' }); // TODO(student): замените на корректный локатор
  ...
  test('Проверить уведомления на странице', async ({ page }) => {
    const successAlert = page.getByText('Успех! Действие выполнено успешно'); // TODO(student): замените на корректный локатор
